...
public class Diagram extends JFrame{
		...
		Thread  thread = new Thread() {
			...
			public void run() {
				while (true) {
					...
					g.fillRect(0, 0, boardWidth, boardHeight);      //vẽ hình chữ nhật màu xám
					
					for (Layer layer : layers) {
						if (layer.isVisible()) 
                                                    layer.drawShape(g);         //vẽ khung shape
					}
					
					g = board.getGraphics();
					g.drawImage(vi, 0, 0, boardWidth, boardHeight, null);       
					//đóng cửa sổ đã bị đóng
					g.dispose();        
				}
			}
		};
	...
	public void reOrganized() {
		...
		layers.clear(); //xóa tất cả các phần tử có trong ArrayList 
