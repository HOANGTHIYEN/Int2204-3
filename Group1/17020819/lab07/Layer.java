...
public class Layer extends JPanel{
	...
	//getter kích cỡ của Shape
	public int getElementsSize() {
		return figure.size();
	}
	//thêm Shape vào ArrayList
	...
	//xóa các Shape giống nhau(cùng kích cỡ,hình dáng)
	...
	public void drawShape(Graphics g) {
		for (Shape shape: figure) {
			...
			if (shape instanceof Rectangle) {
				...
				g.drawRect(pX, pY, w, l);       //vẽ hình chữ nhật
			}
			if (shape instanceof Square) {
				int s  = ((Square) shape).getWidth();
				g.setColor(shape.getColor());
				g.drawRect(pX, pY, s, s);       //vẽ hình vuông dựa trên drawRect(X,Y,width,length)
			}
			if (shape instanceof Circle) {
				int s = ((Circle) shape).getRadius();
				g.setColor(shape.getColor());
				g.drawOval(pX, pY, s, s);       //vẽ hình tròn
			}
